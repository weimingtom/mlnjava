...
jobjectArray newLuaValueArrayFromStack(JNIEnv *env, lua_State *L, int count, int stackoffset) {
    ...
    p = (*env)->NewObjectArray(env, count, LuaValue, NULL);
    for (i = 0; i < count; i++) {
...
void pushUserdataFromJUD(JNIEnv *env, lua_State *L, jobject obj) {
	...
	lua_lock(L);
    lcn = (jstring) (*env)->GetObjectField(env, obj, LuaUserdata_luaclassName);
    luaclassname = GetString(env, lcn);

    ud = (UDjavaobject) lua_newuserdata(L, sizeof(javaUserdata));
    ud->flag = 0;
...
size_t copy_string(JNIEnv *env, jstring src, char *out, size_t len) {
	size_t slen, copy_len;

	const char *cs = GetString(env, src);
    if (!cs) return 0;
    slen = (size_t) (*env)->GetStringUTFLength(env, src);

    copy_len = slen >= len ? len - 1 : slen;
...
int getThrowableMsg(JNIEnv *env, jthrowable t, char *out, size_t len) {
    ...
    ret = (jstring) (*env)->CallObjectMethod(env, t, obj__toString);
    if ((*env)->ExceptionCheck(env) || !ret) return -2;

    copy_len = copy_string(env, ret, out, len);
    if (copy_len == 0)
        return -3;
