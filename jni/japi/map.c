...
struct map_
{
    ...
    map_hash f_hash;        
    map_equals f_equals;    
    map_free_kv f_free_k;   
    map_free_kv f_free_v;   
    int _c;                 
    map_alloc f_alloc;      
#if defined(J_API_INFO)
    sizeof_kv f_sizeof_k;   
    sizeof_kv f_sizeof_v;   
    size_t _mem;            
#endif
};
...
void * map_get(Map *map, const void *key) {
	...
	if (map_error(map)) return NULL;
    hash = map->f_hash(key);
    i = map_index(map, hash);
    entry = map->table[i];
    if (!entry) return NULL;
    e = find(map, entry, key, hash);
    if (e) return e->value;
    return NULL;
}
...
void * map_remove(Map * map, const void * key) {
    ...
    pre = entry;
    e = find_with_pre(map, entry, &pre, key, hash);
    if (e) {
		void * ret;
