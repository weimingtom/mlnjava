...
#define iso_setobj(obj1, obj2) \
    { const TValue *io2=(obj2); TValue *io1=(obj1); \
      val_(io1) = val_(io2); tt_(io1) = tt_(io2); \
      }
...
static int copy_function(lua_State *L, P_Buffer *buf, TValue *val) {
    ...
    f = getproto(val);
    ret = luaU_dump(L, f, writer, buf, 0);
    if (ret) {
        if (buf->buffer)
...
static int openGlobal(lua_State *L) {
    ...
    arg = (Thread_Arg *) iso_malloc(NULL, 0, sizeof(Thread_Arg));
    if (!arg) {
...
static int mln_thread_java_main(lua_State *L, void *ud) {
	mln_java_thread_ctx *ctx;
	void *retvalue;
	typedef void *(*callback_func)(void *);
	callback_func callback;

	if (!ud) return -1;
    ctx = (mln_java_thread_ctx *)ud;
